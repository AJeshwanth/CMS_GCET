...
export default function DepartmentDashboard() {
  ...
  const issue = { id: localStorage.getItem("issue") };
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/complaints`)
      .then(res => res.json())
      .then((data) => {
        // ✅ Filter complaints by department issue category
        const filtered = data.filter(c => c.category === issue.id);
        setComplaints(filtered);
      })
      .catch(() => addNotification('Error fetching complaints', 'error'));
  }, [addNotification, issue.id]);
